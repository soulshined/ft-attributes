...
final class Size implements ValidationAware {
    ...
    public function validate(Property $property, mixed $value): ?IllegalArgumentException
    {
        ...
        if ($transposed_count < $this->min)
            return new IllegalArgumentException($this->message ?? $property->get_qualified_name() . " size is less than expected ($this->min)");
        if ($transposed_count > $this->max)
            return new IllegalArgumentException($this->message ?? $property->get_qualified_name() . " size is greater than expected ($this->max)");
